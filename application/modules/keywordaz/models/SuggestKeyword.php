<?php
class Keywordaz_Model_SuggestKeyword extends Db_Abstract{
    ...
    public function getSuggestKeyword()
    {
        ...
        for($i = 0; $i < $cnt; $i++)
        {
            if ($this->suggestAry[$i] == ""){
       		...
       		$client->setUri(Com_Const::API_AMAZON.urlencode($newKeyword));
    ...
    public function saveResult($result){
        ...
        $fp = fopen("history/archive_az/".str_replace(array("/", " ", ":", "-"), "", $this->registdt)."_".$keyword.".html", 'w');
        fwrite($fp, $result);
        fclose($fp);        
    }
    ...
    public function getResultFile($fileName) {
        ...
        $resultFile = @file_get_contents("history/archive_az/".$fileName.".html");
        return $resultFile;        
    }
    ...
    public function getSuggestKeywordAjax()
    {
        ...
        $client->setUri(Com_Const::API_AMAZON.urlencode($this->keyword));
