<?php
class Keywordaz_Model_Csv extends Db_Abstract{
    ...
    public function getCsvOrderList($userid) {
        $dao = new Db_CsvOrderUser();
        $orders = $dao->getCsvOrderList($userid, Com_Const::AMAZON);
        return $orders;
    }
    
    public function getNotExecuteCsvId($usreid) {
        $dao = new Db_CsvOrderUser();
        $rows = $dao->getNotExecuteCsv($usreid, Com_Const::AMAZON);
        if (count($rows)) {
    ...
    public function hasExecutingCsv($userid) {
        $dao = new Db_CsvOrderUser();
        $rows = $dao->getExecutingCsv($userid, Com_Const::AMAZON);
        if (count($rows) == 0) {
            return false;
        }
        return true;
    ...
    {
        $searchHistoryEntity = new Keywordaz_Model_Entities_SearchHistory();
	...
	public function getExpandResult($historyid){
	    $dao = new Db_ExpandResult();
	    $row = $dao->getRowById($historyid, Com_Const::AMAZON);
	    return $row;
	}
	
	public function deleteOrderCsv($historyid, $userid) {
	    $dao = new Db_CsvOrderUser();
        $dao->deleteOne($historyid, $userid, Com_Const::AMAZON);
        
        $dao = new Db_ExpandResult();
        $dao->deleteOne($historyid, Com_Const::AMAZON);
	}
	...
	public function expandLevel($skStr, $startTime, $interruptRst, $interruptGrpNm) {
	    ...
	    // $api = Com_Const::API_AMAZON;
	    // $api = "http://www.kword.lrv.jp/result";
	    // $api = "https://medical-blog.net/kw/result";
        
        $commonapi = array("url" => Com_Const::API_AMAZON,  "per" => 25);
        $sakura = array("url" => "http://ad8-ssl.sakura.ne.jp/kw/result", "per" => 25);
        $x = array("url" => "http://medical-blog.net/kw/result",  "per" => 20);
        $leo = array("url" => "http://www.kword.lrv.jp/result",  "per" => 20);
        $core = array("url" => "http://kword.coresv.com/kw/result",  "per" => 10);

        $urls = array($commonapi, $sakura, $x, $leo, $core);
	    ...
	    foreach ($skAry as $groupNm => $keyword) {
	        ...
	        $posts = array(
	                'site' => Com_Const::AMAZON,
	                'keyword' => urlencode($keyword)
	        );
	        
	        try {
	            if ($api === Com_Const::API_AMAZON) {
	                $client->setUri($api.urlencode($keyword));
	                $response = Com_Util::sendAPIRequest($client, Com_Const::AMAZON);
	            } else {
	                // other server
	                $client->setUri($api);
	                $client->setParameterPost($posts);
	                $response = Com_Util::sendAPIRequest($client, Com_Const::AMAZON, "POST");
	            }
	        ...
	        if (!$response || in_array($response, array(Com_Const::FORBIDDEN, Com_Const::SERVICEUNAVAILABLE, Com_Const::EAPPIDERR)) ) {
	            ...
	            $otherServerRes = Com_Util::getFromReplaceServer($client, $posts, Com_Const::AMAZON);
	            if ($otherServerRes == false) {
	                 
	                Com_Log::registApiErrorLog("game", "over", "0件", mb_convert_encoding($keyword, "shift-jis"), Com_Const::AMAZON);
	        ...
	        if ($response == null) {
                // some error
                Com_Log::registApiErrorLog("some error", "over", $api, $keyword, Com_Const::AMAZON);
	...
	private function parseXMLResponseCsv($xmlContents, $groupNm, $keyword) {
	    ...
	    if ($cnt == 0) {
	        //Com_Log::registApiErrorLog($xmlContents, "parseXMLResponseCsv", "0件", $keyword, Com_Const::AMAZON);
	        return "";
	    }
