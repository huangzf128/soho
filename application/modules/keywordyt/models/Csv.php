<?php
class Keywordyt_Model_Csv extends Db_Abstract{
    ...
    public function getCsvOrderList($userid) {
        $dao = new Db_CsvOrderUser();
        $orders = $dao->getCsvOrderList($userid, Com_Const::YOUTUBE);
        return $orders;
    }
    
    public function getNotExecuteCsvId($usreid) {
        $dao = new Db_CsvOrderUser();
        $rows = $dao->getNotExecuteCsv($usreid, Com_Const::YOUTUBE);
        if (count($rows)) {
    ...
    public function hasExecutingCsv($userid) {
        $dao = new Db_CsvOrderUser();
        $rows = $dao->getExecutingCsv($userid, Com_Const::YOUTUBE);
        if (count($rows) == 0) {
            return false;
        }
        return true;
    ...
    {
        $searchHistoryEntity = new Keywordyt_Model_Entities_SearchHistory();
	...
	public function getExpandResult($historyid){
	    $dao = new Db_ExpandResult();
	    $row = $dao->getRowById($historyid, Com_Const::YOUTUBE);
	    return $row;
	}
	
	public function deleteOrderCsv($historyid, $userid) {
	    $dao = new Db_CsvOrderUser();
        $dao->deleteOne($historyid, $userid, Com_Const::YOUTUBE);
        
        $dao = new Db_ExpandResult();
        $dao->deleteOne($historyid, Com_Const::YOUTUBE);
	}
	...
	public function expandLevel($skStr, $startTime, $interruptRst, $interruptGrpNm) {
	    ...
	    // $api = Com_Const::API_YOUTUBE;
	    // $api = "http://www.kword.lrv.jp/result";
	    
	    $commonapi = array("url" => Com_Const::API_YOUTUBE,  "per" => 25);
	    $leo = array("url" => "http://www.kword.lrv.jp/result",  "per" => 25);
	    ...
	    $urls = array($commonapi, $leo, $x, $core, $sakura);
	    ...
	    foreach ($skAry as $groupNm => $keyword) {
	        ...
	        $posts = array(
	                'site' => Com_Const::YOUTUBE,
	                'keyword' => urlencode($keyword)
	        );

	        try {
	            
	            if ($api === Com_Const::API_YOUTUBE) {
	                $client->setUri($api.urlencode($keyword));
	                $response = Com_Util::sendAPIRequest($client, Com_Const::YOUTUBE);
	            } else {
	                // other server
	                $client->setUri($api);
	                $client->setParameterPost($posts);
	                $response = Com_Util::sendAPIRequest($client, Com_Const::YOUTUBE, "POST");
	            }
	        ...
	        if (!$response || in_array($response, array(Com_Const::FORBIDDEN, Com_Const::SERVICEUNAVAILABLE, Com_Const::EAPPIDERR)) ) {
	        	...
	        	$otherServerRes = Com_Util::getFromReplaceServer($client, $posts, Com_Const::YOUTUBE);
	            if ($otherServerRes == false) {
	                
	                Com_Log::registApiErrorLog("game", "over", "0件", mb_convert_encoding($keyword, "shift-jis"), Com_Const::YOUTUBE);
	        ...
	        if ($response == null) {
	            // some error
	            Com_Log::registApiErrorLog("some error", "over", $api, $keyword, Com_Const::YOUTUBE);
	            
	            // return Com_Const::ERROR;
	        } else {
	            $response = mb_convert_encoding($response, "utf-8","UTF-8,eucjp-win,sjis-win");
	...
	private function parseXMLResponseCsv($xmlContents, $groupNm, $keyword) {
	    ...
	    if ($cnt == 0) {
	        // Com_Log::registApiErrorLog($xmlContents, "parseXMLResponseCsv", "0件", $keyword, Com_Const::YOUTUBE);
	        return "";
	    }
