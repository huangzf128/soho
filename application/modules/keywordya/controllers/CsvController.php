...
class Keywordya_CsvController extends Zend_Controller_Action
{
    private $userid = null;
    ...
    public function indexAction()
    {
        if (isset($this->userid)) {
            $csvModel = new Keywordya_Model_Csv();
            $orders = $csvModel->getCsvOrderList($this->userid);
            $this->view->orders = $orders;
        } else {
            $this->_redirect('/keywordya/index/login');
        }
    }
    ...
    public function csvFileOrderAction()
    {
        ...
        try {
            ...
            if ($result) {
                // 非同期処理
                if (Com_Util::isHttps()) {
                    $url_list = array("https://".$_SERVER['HTTP_HOST']."/keywordya/csv/expand-Result-Ajax?historyid=".$historyid."&userid=".$this->userid);
                } else {
                    $url_list = array("http://".$_SERVER['HTTP_HOST']."/keywordya/csv/expand-Result-Ajax?historyid=".$historyid."&userid=".$this->userid);
                }                
                Com_Util::sendMulitRequest($url_list, 2);
            }
        } catch (Exception $e) {
            Com_Log::registErrorLog($e->getMessage(), "csvFileOrderAction", null, null, Com_Const::YAHOO);
        }
    ...
    public function expandInterruptedResultAjaxAction() {
        ...
        if ($interruptinfo[2] === $rand) {
            ...
            if ($callApiTimes > 490) {
                ...
                // Com_Log::registExpandLog($callApiTimes, "Sleep", "historyid: ".$historyid, null, Com_Const::YAHOO);
            ...
            try {
                // Com_Log::registExpandLog($interruptinfo[0]."--".$interruptinfo[1], "Interrupted", "historyid: ".$historyid, $rand, Com_Const::YAHOO);
                ...
                Com_Log::registErrorLog($e->getMessage(), "expandResultAjaxAction: catch ", "historyid: ".$historyid, null, Com_Const::YAHOO);
            }
        }
    }
    ...
    private function callInterruptedRequest($historyid, $rand) {
        // 中断
        if (Com_Util::isHttps()) {
            $url_list = array("https://".$_SERVER['HTTP_HOST']."/keywordya/csv/expand-Interrupted-Result-Ajax?historyid=".$historyid."&userid=".$this->userid."&rand=".$rand);
        } else {
            $url_list = array("http://".$_SERVER['HTTP_HOST']."/keywordya/csv/expand-Interrupted-Result-Ajax?historyid=".$historyid."&userid=".$this->userid."&rand=".$rand);
        }
        Com_Util::sendMulitRequest($url_list, 2);
    }
    ...
    private function createExpandCsv($historyid, $startTime, $csvModel) {
        ...
        try {
            ...
            for ($i = 0; $i < Com_Const::CSV_EXPAND_LEVEL_MAX; $i++) {
                ...
                if ($this->isNeedExpand($expandRst, $i + 1)) {
                    ...
                    if (empty($interruptinfo[1])) {
                        Com_Log::registExpandLog("start: level".($i + 1), $historyid, null, null, Com_Const::YAHOO);
                    }
            ...
            Com_Log::registExpandLog("end", $historyid, null, null, Com_Const::YAHOO);
            
            $csvModel->updateExpandStatus($historyid, Com_Const::STATUS_FINISH);
        } catch (Exception $e) {
            
            $csvModel->updateExpandStatus($historyid, Com_Const::STATUS_ERROR);
            Com_Log::registErrorLog($e->getMessage(), "createExpandCsv: catch ", "historyid: ".$historyid, null, Com_Const::YAHOO);
            return false;
        }
