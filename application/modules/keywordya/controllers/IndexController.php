...
class Keywordya_IndexController extends Zend_Controller_Action
{
    ...
    public function authAction()
    {
        ...
        $model = new Keywordya_Model_User();
...
//             $zend_session->type = Com_Const::USER_COMMON;
    ...
    * Delete
    ...
    public function displayDeleteAction(){
    	...
    	if(!empty($keyword)){
    		$model = new Keywordya_Model_RecoveryModel($keyword);
    ...
    // ファイルを元にDBを復旧する(機能クローズ)
//     public function recoveryAction(){
//     	$this->_helper->layout->disableLayout();
//     	$start = trim(mb_convert_encoding($this->getRequest()->start, "utf-8", "UTF-8,eucjp-win,sjis-win"));
//     	$end = trim(mb_convert_encoding($this->getRequest()->end, "utf-8", "UTF-8,eucjp-win,sjis-win"));
//     	if(!empty($start) && !empty($end)){
//     		$model = new Keywordya_Model_RecoveryModel($keyword);
//     		$result = $model->recovery($start, $end);
//     		if($result){
//     			$this->view->msg = "リカバリーが成功しました。";
//     		} else {
//     			$this->view->msg = "リカバリーが失敗しました。";
//     		}
//     	}else{
//     		$this->view->msg = "復旧期間を入力してください。";
//     	}
