<?php
class Keywordbs_Model_Csv extends Db_Abstract{
    ...
    public function getCsvOrderList($userid) {
        $dao = new Db_CsvOrderUser();
        $orders = $dao->getCsvOrderList($userid, Com_Const::BING);
        return $orders;
    }
    
    public function getNotExecuteCsvId($usreid) {
        $dao = new Db_CsvOrderUser();
        $rows = $dao->getNotExecuteCsv($usreid, Com_Const::BING);
        if (count($rows)) {
    ...
    public function hasExecutingCsv($userid) {
        $dao = new Db_CsvOrderUser();
        $rows = $dao->getExecutingCsv($userid, Com_Const::BING);
        if (count($rows) == 0) {
            return false;
        }
        return true;
	...
	public function getExpandResult($historyid){
	    $dao = new Db_ExpandResult();
	    $row = $dao->getRowById($historyid, Com_Const::BING);
	    return $row;
	}
	
	public function deleteOrderCsv($historyid, $userid) {
	    $dao = new Db_CsvOrderUser();
        $dao->deleteOne($historyid, $userid, Com_Const::BING);
        
        $dao = new Db_ExpandResult();
        $dao->deleteOne($historyid, Com_Const::BING);
	}
	...
	public function expandLevel($skStr, $startTime, $interruptRst, $interruptGrpNm) {
	    ...
	    // $api = Com_Const::API_BING;
	    // $api = "http://www.kword.lrv.jp/result";
	    
	    $commonapi = array("url" => Com_Const::API_BING,  "per" => 25);
	    $core = array("url" => "http://kword.coresv.com/kw/result",  "per" => 25);
	    $x = array("url" => "http://medical-blog.net/kw/result",  "per" => 20);
	    $sakura = array("url" => "http://ad8-ssl.sakura.ne.jp/kw/result", "per" => 20);
	    $leo = array("url" => "http://www.kword.lrv.jp/result",  "per" => 10);
	    
	    $urls = array($commonapi, $core, $x, $sakura, $leo);
	    ...
	    foreach ($skAry as $groupNm => $keyword) {
	        ...
	        try {
	            if ($api === Com_Const::API_BING) {
	                $client->setUri($api.urlencode($keyword));
	                $response = Com_Util::sendAPIRequest($client, Com_Const::BING);
	            } else {
	                // other server
	                $client->setUri($api);
	                $client->setParameterPost($posts);
	                $response = Com_Util::sendAPIRequest($client, Com_Const::BING, "POST");
	            }
	        ...
	        if (!$response || in_array($response, array(Com_Const::FORBIDDEN, Com_Const::SERVICEUNAVAILABLE, Com_Const::EAPPIDERR)) ) {
	            ...
	            $otherServerRes = Com_Util::getFromReplaceServer($client, $posts, Com_Const::BING);
	            if ($otherServerRes == false) {
	                
	                Com_Log::registApiErrorLog("game", "over", "0件", mb_convert_encoding($keyword, "shift-jis"), Com_Const::BING);
            ...
            if ($response == Com_Const::SERVICEUNAVAILABLE) {
                 
                return array(Com_Const::INTERRUPTION, $expandKeywords, $groupNm, 491);

	        } elseif ($response == null) {
	            // some error
	            Com_Log::registApiErrorLog("some error", "over", $api, $keyword, Com_Const::BING);
	...
	private function parseXMLResponseCsv($xmlContents, $groupNm, $keyword) {
	    ...
	    if ($cnt == 0) {
	        //Com_Log::registApiErrorLog($xmlContents, "parseXMLResponseCsv", "0件", $keyword, Com_Const::BING);
	        return "";
	    }
	    ...
	    foreach($suggestKeyword as $key => $val) {
	        ...
	        if($val != null && trim($val["Txt"]) != "" && $keyword != $val["Txt"]){
                $expandKeywords .= Com_Util::filter($val["Txt"])."&";
	        }
	    }
